...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("https://av1-deploy-back-1.onrender.com");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("https://av1-deploy-back-1.onrender.com",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`https://av1-deploy-back-1.onrender.com/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
